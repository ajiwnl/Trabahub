...
namespace Trabahub.Controllers
{
    public class ListingController : Controller
    {
        ...
        public async Task<IActionResult> ChargeGCash(string gcashprice, string gcashdesc, string gcashin, string gcashout, string gcashchoice, string gcashdynamic, string status)
        {
            ...
            if (gcashprice == null || gcashdesc == null || gcashin == null || gcashout == null || gcashchoice == null || gcashdynamic == null)
            {
        ...
        public JsonResult GetDailyData()
        {
            ...
            if (role == "Owner")
            {
                ...
                // Query all listings belonging to the logged-in owner
                var ownerListings = _context.Listing.Where(l => l.OwnerUsername == ownerUsername).ToList();
                int totalBooks = 0;


                // Initialize total listings count and dictionary to store total books count for each establishment
                int totalListings = ownerListings.Count;
        ...
        public JsonResult UpdateCount()
        {
            ...
            if (role == "Client" || role == "Owner")
            {
                ...
                if (listing != null)
                {
                    ...
                    var activeBookings = _context.Booking.Where(b => b.ESTABNAME == currentListing && b.Status == "Active");
            ...
            {
                var ownedEstablishments = _context.Listing.Where(l => l.OwnerUsername == ownerUsername).ToList();

                int totalAccommodationCount = 0;

                foreach (var establishment in ownedEstablishments)
                {
                    var activeBookings = _context.Booking.Where(b => b.ESTABNAME == establishment.ESTABNAME && b.Status == "Active");

                    foreach (var booking in activeBookings)
                    {
                        if (booking.ENDTIME <= DateTime.Now && booking.Status != "Finished")
                        {
                            // Increase the accommodation count for the current establishment
                            establishment.ACCOMODATION++;
                            booking.Status = "Finished";
                        }
                    }

                    totalAccommodationCount += establishment.ACCOMODATION ?? 0;
                }

                _context.SaveChanges();

                return Json(totalAccommodationCount);
            }

            else if (role == "Admin")
            {
                var establishments = _context.Listing.ToList();

                int totalAccommodationCount = 0;

                foreach (var establishment in establishments)
                {
                    var activeBookings = _context.Booking.Where(b => b.Status == "Active" && b.ESTABNAME == establishment.ESTABNAME);
